...
#                     echo "        model.set${field_name^}(Static$field_type.${lowercase_field_type}Empty());"  >> "$static_file" ;;
#             esac
#         fi
#     done
...
#     echo "" >> "$static_file"
#     echo "    public static ${class_name} ${lowercase_model_name}Empty() {" >> "$static_file"
#     echo "        ${class_name} model = new ${class_name}();" >> "$static_file"
#     echo "        model.setId(ID);" >> "$static_file"
#     echo "        return model;" >> "$static_file"
#     echo "    }" >> "$static_file"
    ...
    while IFS= read -r line; do
        if [[ $line == *"private "* ]]; then
            ...
            if [ "$field_count" -eq 0 ]; then
                echo "                .andExpect(jsonPath(\"$.${var_name}\").value(${lowercase_model_name}Response.get${uppercase_var_name}()));" >> "temp"
            else
                echo "                .andExpect(jsonPath(\"$.${var_name}\").value(${lowercase_model_name}Response.get${uppercase_var_name}()))" >> "temp"
            fi
        fi
